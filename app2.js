...
const formfile = fs.readFileSync('./views/form.html','utf-8')
const datauser = (fs.readFileSync('./json/details.json','utf-8'))
const datahtml = fs.readFileSync('./views/userdata.html','utf-8')
const userjson =JSON.parse(datauser)
 let userdata=userjson.map((item)=>{
    ...
    output=output.replace('{{%phno}}',item.phonenumber)
    output=output.replace('{{%bgroup}}',item.bloodgroup)
    return output
})
